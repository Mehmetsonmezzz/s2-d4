...
function CemberinAlani(yariCap, _pi) {
  _pi = pi;
  return _pi * Math.pow(yariCap, 2);
}
...
sayilar.forEach(tamBolum);
function tamBolum(item) {
  if (item % 3 == 0) {
...
//3d çözümü
//3d. `besyuzdenkucuksayilar` adında bir dizi oluşturarak, sayilar dizisinin içindeki 500'den küçük sayıları bu diziye atayın (.filter metodunu kullanın)
let besyuzdenkucuksayilar = sayilar.filter(checkFiveHundred);

function checkFiveHundred(number) {
  return number < 500;
}

console.log("besyuz", besyuzdenkucuksayilar);
